...
export default async function WeeklyPage() {
  const [channelMatrix, productMatrix] = await Promise.all([
  ...
  const weeks = weeklyTrend(channels);
  const storeCompare = storeWeeklyComparison(channels).slice(0, 10);
  const attention = needAttentionStores(channels);
  const topProducts = productTop10(products);
  const review = weeklyReview(channels);
    ...
    <main className="min-h-screen p-6">
      <div className="mx-auto max-w-7xl space-y-6">
        ...
        <section className="grid gap-4 md:grid-cols-4">
          <Kpi title="월 목표" value={formatWon(totals.target)} />
          <Kpi title="누적 매출" value={formatWon(totals.total)} />
          <Kpi title="목표 달성률" value={pct(totals.achievementRate)} />
          <Kpi title="착지예측 매출(달성률)" value={formatWon(totals.forecast)} sub={pct(totals.forecastRate)} />
        </section>
            ...
            <div className="h-80">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={storeCompare}>
                  <CartesianGrid strokeDasharray="3 3" />
                  <XAxis dataKey="storeName" tick={{ fontSize: 11 }} interval={0} angle={-20} textAnchor="end" height={70} />
                  <YAxis tickFormatter={(v) => `${Math.round(Number(v) / 10000000)}천만`} />
                  <Tooltip formatter={(v: any) => formatWon(Number(v))} />
                  <Bar dataKey="prevWeek" name="전주" fill="#cbd5e1" />
                  <Bar dataKey="thisWeek" name="금주" fill="#111827" />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </Card>
                ...
                <div key={s.storeName} className="rounded-2xl border border-slate-100 bg-slate-50 p-4">
                  <div className="flex items-center justify-between gap-3">
                    ...
                    <div className="text-right">
                      <p className={`text-xl font-black ${s.changeRate < 0 ? "text-red-600" : "text-blue-600"}`}>
                        {s.changeRate >= 0 ? "+" : ""}
                        {pct(s.changeRate)}
                      </p>
                  ...
                  <div className="mt-3 grid grid-cols-3 gap-2 text-sm">
                    <Mini label="전주 매출" value={formatWon(s.prevWeek)} />
                    <Mini label="금주 매출" value={formatWon(s.thisWeek)} bold />
                    <Mini label="착지예측" value={pct(s.forecastRate)} />
                  </div>
                </div>
              ))}
            </div>
              ...
              {topProducts.map((p, i) => (
                <div key={p.styleCode || p.productName} className="rounded-2xl border border-slate-100 bg-white p-4 shadow-sm">
                  <div className="flex justify-between gap-4">
                    ...
                    <div className="text-right">
                      <p className="text-lg font-black">{p.weekSold.toLocaleString("ko-KR")}개</p>
                      <p className="text-xs text-slate-500">금주 판매량</p>
                    </div>
                  </div>
                  <div className="mt-3 grid grid-cols-2 gap-2 text-sm md:grid-cols-4">
                    <Mini label="전주 판매량" value={`${p.prevSold.toLocaleString("ko-KR")}개`} />
                    <Mini label="증감률" value={`${p.changeRate >= 0 ? "+" : ""}${pct(p.changeRate)}`} />
                    <Mini label="판매금액" value={formatWon(p.weekAmount || p.salesAmount)} />
                    <Mini label="판매율" value={pct(p.salesRate)} />
                  </div>
                </div>
              ))}
            </div>
        ...
        <section className="grid gap-6 lg:grid-cols-2">
          <Card title="주간 매출 리뷰">
            <ul className="space-y-3 text-sm leading-6 text-slate-700">
              {review.review.map((text) => <li key={text}>• {text}</li>)}
            </ul>
          </Card>
          <Card title="운영 제안">
            <ul className="space-y-3 text-sm leading-6 text-slate-700">
              {review.suggestions.map((text) => <li key={text}>• {text}</li>)}
            </ul>
          </Card>
        </section>
      </div>
...
function Mini({ label, value, bold = false }: { label: string; value: string; bold?: boolean }) {
  return (
    <div className="rounded-xl bg-slate-100 p-3">
      <p className="text-xs text-slate-500">{label}</p>
      <p className={`mt-1 ${bold ? "font-black text-slate-950" : "font-bold text-slate-700"}`}>{value}</p>
    </div>
  );
}
